...
public class ParticlesRenderer
{
	...
	public ParticleData addParticle(ParticleType particleType, Location location)
	{
		ParticleData particle = particleType.createNew(location.getWorld(), (float)location.getX(), (float)location.getY(), (float)location.getZ());
		
		return addParticle(particleType, particle);
	}
	
	public ParticleData addParticle(ParticleType particleType, ParticleData particle)
	{
	...
	public int render(RenderingContext renderingContext)
	{
		...
		for(ParticleType particleType : particles.keySet())
		{
			...
			//{
				...
				if (particles.get(particleType).size() > 0)
				{
					...
					while(iterator.hasNext())
					{
						...
						if(p instanceof ParticleTextureCoordinates)
						{
							/*
							 * 
			fb.put(new float[] { 1, 1 });
			...
			fb.put(new float[] { -1, 1 });
							...
							ParticleTextureCoordinates texCoords = (ParticleTextureCoordinates)p;
