...
public class VoxelFormat
{
	...
	// It stores all voxels in 32-bit signed ( Java's fault :c ) ints
	...
	public static void main(String a[])
	{
		...
		System.out.println("BlockID : " + id(data) + "Meta : " + meta(data)
				+ "Sun : " + sunlight(data) + "Block : " + blocklight(data));
	}

	public static int format(int blockID, int metadata, int sunlight,
			int blocklight)
	{
		blockID %= 0x10000;
		metadata %= 0x10;
		...
		return blockID | metadata << 0x10 | sunlight << 0x14
				| blocklight << 0x18;
	}
