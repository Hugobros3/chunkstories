...
public class TrueTypeFontRenderer
{
		...
		while (i < whatchars.length())
		{
			...
			if (glyph != null)
			{
				...
				else
				{
					if(target == null)
					{
						renderingContext.getGuiRenderer().setState(pageTexture.getId(), true, true, colorModified);
						drawQuad(totalwidth * scaleX + x, startY * scaleY + y, (glyph.width) * scaleX, glyph.height * scaleY, ((float)glyph.x) / textureWidth, ((float)(glyph.y + glyph.height)) / textureHeight, ((float)(glyph.x + glyph.width)) / textureWidth, ((float)glyph.y) / textureHeight);
					}
					else
					{
	...
	private void drawQuad(float startX, float startY, float width, float height, float srcX, float srcY, float srcX2, float srcY2)
	{
		renderingContext.getGuiRenderer().drawBoxWindowsSpace(startX, startY, startX + width, startY + height, srcX, srcY, srcX2, srcY2, -1, false, true, null);
	}
}
