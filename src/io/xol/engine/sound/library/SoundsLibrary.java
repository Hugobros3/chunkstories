...
public class SoundsLibrary
{
	...
	{
		SoundDataOggSample sd = new SoundDataOggSample(DefaultModsManager.getAsset(soundEffect));
		sd.name = soundEffect;
		if(sd.loadedOk())
			return sd;
		return null;
	...
	private static SoundDataBuffered obtainOggStream(String musicName)
	{
		...
		sd = new SoundDataOggStream(DefaultModsManager.getAsset(musicName).read());
		sd.name = musicName;
		if(sd.loadedOk())
			return sd;
