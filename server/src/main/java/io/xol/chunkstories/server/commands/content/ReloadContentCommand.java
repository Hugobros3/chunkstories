...
public class ReloadContentCommand implements CommandHandler {
    ...
    public boolean handleCommand(CommandEmitter emitter, Command command, String[] arguments) {
        if (command.getName().equals("reload") && emitter.hasPermission("content.reload")) {
            if (arguments.length == 0) {
                ...
                emitter.sendMessage("#00FFD0"
                        + "/reload plugins #FFFFD0| Reloads only the plugins ( disables them, loads them anew then re-enables them )");
                emitter.sendMessage("#00FFD0"
                        + "/reload <subsystem> #FFFFD0| Reloads one of the specific hot-reloadable subsystems shown below");
	...
	/*TODO this has to go somewhere in client, or better still be added by the backend/relevant implem code
	private void reloadShaders(CommandEmitter emitter) {
		if (this.gameContext instanceof IngameClient) {
			IngameClient client = (IngameClient) gameContext;
