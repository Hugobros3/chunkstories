...
public class ServerAnnouncerThread extends Thread {
	...
	@Override public void run() {
		try {
			...
			String externalIp = new SimpleWebRequest("https://chunkstories.xyz/api/sayMyName.php?ip=1").waitForResult();
			...
			while (run.get()) {
				if (server.getServerConfig().getBooleanValue("server.announcer.enable")) {
					...
					String postData =
							"srvname=" + serverName + "&desc=" + serverDescription + "&ip=" + externalIp + "&iip=" + internalIp + "&mu=" + server.getHandler()
									.getMaxClients() + "&u=" + server.getHandler().getPlayersNumber() + "&n=0&w=default&p=1&v=" + VersionInfo.version
									+ "&lolcode=" + lolcode;

					new SimplePostRequest("https://chunkstories.xyz/api/serverAnnounce.php", postData);
