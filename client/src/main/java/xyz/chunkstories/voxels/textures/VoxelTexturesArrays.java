...
package xyz.chunkstories.voxels.textures;
	...
	public void buildTextureAtlas() {
		...
		try {
			...
			if (gl_MaxTextureArraySize < 2048) {
				logger().warn(
						"Max texture array size < 2048. For ideal results please use a GPU from this geological era.");
			}
			...
			if (defaultAlbedo == null || defaultNormal == null || defaultMaterial == null) {
				logger().error(
						"Missing 'notex.png' for one of the 3 texture types (albedo, normal, material), exiting !");
				System.exit(-602);
			}
