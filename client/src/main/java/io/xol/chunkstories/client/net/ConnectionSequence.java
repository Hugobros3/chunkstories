		...
		try {
			...
			if (Client.offline) {
				...
				SimplePostRequest spr = new SimplePostRequest("https://chunkstories.xyz/api/serverTokenObtainer.php",
						"username=" + Client.username + "&sessid=" + Client.session_key);
			...
			for (String requiredMod : modsString.split(";")) {
				...
				// requiredMod;
				Client.getInstance().logger().info("Server asks for mod " + modInternalName + " (" + modSizeInBytes
				...
				try {
					...
					Client.getInstance().logger().info("Could not load downloaded mod " + modInternalName + " (hash: "
				...
				if (!actualMd5Hash.equals(modMd5Hash)) {
					Client.getInstance().logger().info("Invalid md5 hash for mod " + modInternalName
