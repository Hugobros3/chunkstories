...
public class MeshRenderer implements RenderableMesh {
	...
	/*
	 * Texture2D instancesDataTexture = new
	 * Texture2DRenderTarget(TextureFormat.RGBA_32F, 32, 32); ByteBuffer
	 * instancesDataBuffer = BufferUtils.createByteBuffer(32 * 32 * 4 * 4);
	 * 
	 * public void renderInstanciated(RenderingContext renderingContext,
	 * Collection<AnimatableData> instancesData) { prepareDraw(renderingContext);
	 * 
	 * int totalTriangles = 0; for (String currentVertexGroup : boneGroups.keySet())
	 * { int trianglesInThisGroup = boneGroups.get(currentVertexGroup);
	 * 
	 * int dataInInstancesBuffer = 0; Iterator<AnimatableData> iterator =
	 * instancesData.iterator(); instancesDataBuffer.clear();
	 * 
	 * //Fill the buffer while (iterator.hasNext()) { AnimatableData data =
	 * iterator.next();
	 * 
	 * if (!data.skeleton.shouldHideBone(renderingContext, currentVertexGroup)) {
	 * Matrix4f animation =
	 * data.skeleton.getBoneHierarchyTransformationMatrixWithOffset(
	 * currentVertexGroup, data.animationTime);
	 * 
	 ...
	 * 
	 * instancesDataBuffer.putFloat(mat.m00); instancesDataBuffer.putFloat(mat.m01);
	 * instancesDataBuffer.putFloat(mat.m02); instancesDataBuffer.putFloat(mat.m03);
	 * instancesDataBuffer.putFloat(mat.m10); instancesDataBuffer.putFloat(mat.m11);
	 * instancesDataBuffer.putFloat(mat.m12); instancesDataBuffer.putFloat(mat.m13);
	 * instancesDataBuffer.putFloat(mat.m20); instancesDataBuffer.putFloat(mat.m21);
	 * instancesDataBuffer.putFloat(mat.m22); instancesDataBuffer.putFloat(mat.m23);
	 * instancesDataBuffer.putFloat(mat.m30); instancesDataBuffer.putFloat(mat.m31);
	 * instancesDataBuffer.putFloat(mat.m32); instancesDataBuffer.putFloat(mat.m33);
	 * 
	 * instancesDataBuffer.putFloat(data.sunLight);
	 * instancesDataBuffer.putFloat(data.blockLight);
	 * 
	 * for (int f = 0; f < 14; f++) instancesDataBuffer.putFloat(0.0f);
	 * 
	 * dataInInstancesBuffer += 8; }
	 * 
	 * //Don't overfill the buffer while (dataInInstancesBuffer == 1024) {
	 * instancesDataBuffer.flip(); instancesDataTexture.uploadTextureData(32, 32,
	 * instancesDataBuffer); instancesDataBuffer.clear();
	 * drawInstanceBufferContents(renderingContext, totalTriangles * 3,
	 * trianglesInThisGroup * 3, dataInInstancesBuffer / 4); dataInInstancesBuffer =
	 * 0; } }
	 * 
	 * //Render in bulk if (dataInInstancesBuffer > 0) { //Pad the buffer int k =
	 * dataInInstancesBuffer; while (k < 1024) { for (int f = 0; f < 32; f++)
	 * instancesDataBuffer.putFloat(0.0f); k += 8; }
	 * 
	 * instancesDataBuffer.flip(); instancesDataTexture.uploadTextureData(32, 32,
	 * instancesDataBuffer); instancesDataBuffer.clear();
	 * 
	 * drawInstanceBufferContents(renderingContext, totalTriangles * 3,
	 * trianglesInThisGroup * 3, dataInInstancesBuffer / 4); }
	 * 
	 * totalTriangles += trianglesInThisGroup; } }
	 */

	/*
	 * private void drawInstanceBufferContents(RenderingContext renderingContext,
	 * int start, int count, int dataInInstancesBuffer) {
	 * renderingContext.bindTexture2D("instancedDataSampler", instancesDataTexture);
	 * //renderingContext.currentShader().setUniformSampler(13,
	 * "instancedDataSampler", instancesDataTexture);
	 * 
	 ...
	 * GLCalls.drawArraysInstanced(GL_TRIANGLES, start, count,
	 * dataInInstancesBuffer);
