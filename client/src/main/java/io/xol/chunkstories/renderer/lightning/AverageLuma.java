...
public class AverageLuma {
	...
	public void computeAverageLuma(Texture2DRenderTargetGL finalBuffer) {
		...
		if (illuminationDownloadInProgress == null) {
			...
			// To avoid crashing when the windows size change, we send the PBO copy request
			// immediately and instead wait to look for it
			this.illuminationDownloadInProgress = this.illuminationDownloader.copyTexure(finalBuffer, maxMipLevel);

			// We remember the size of the data to expect since that may change due to
			// windows resizes and whatnot
			...
			if (frameLumaDownloadDelay > 0)
				...
				if (minMipmapBuffer != null) {
					...
					for (int i = 0; i < downloadSize; i++)
						averageColorForAllFrame.add(minMipmapBuffer.getFloat(), minMipmapBuffer.getFloat(),
								minMipmapBuffer.getFloat());
		...
		float luma = averageColorForAllFrame.x() * 0.2125f + averageColorForAllFrame.y() * 0.7154f
				+ averageColorForAllFrame.z() * 0.0721f;
