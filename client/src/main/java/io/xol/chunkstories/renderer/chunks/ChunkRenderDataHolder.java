...
/**
 * Responsible of holding all rendering information about one chunk ie : VBO
 * creation, uploading and deletion, as well as decals TODO actually handle
 * decals here retard
 */
public class ChunkRenderDataHolder implements ChunkMeshUpdater {
	 ...
	 * //if(chunk.chunkZ != 5) // return;
	 * FakeImmediateModeDebugRenderer.glColor4f(5, 0, (float) Math.random() * 0.01f,
	 * 1); SelectionRenderer.cubeVertices(chunk.getChunkX() * 32 + 16,
	 * chunk.getChunkY() * 32, chunk.getChunkZ() * 32 + 16, 32, 32, 32); }
	 */
	...
	public void setData(ChunkMeshDataSections newData) {
		...
		// No verticesObject already created; create one, fill it and then change the
		// bails
		if (verticesObject == null) {
		...
		// Already a VerticesObject present hum, we create another one then delete the
		// old one
		else {
			VertexBuffer wip = worldRenderer.getRenderingInterface().newVertexBuffer();
			Fence fence = wip.uploadData(newData.dataToUpload);
	...
	public int renderSections(RenderingInterface renderingContext, RenderLodLevel renderLodLevel,
	...
	public int renderSection(RenderingInterface renderer, VertexLayout vertexLayout, RenderLodLevel renderLodLevel,
			ShadingType renderPass) {
		...
		case WHOLE_BLOCKS:
			...
			// Vertex data : [VERTEX_POS(4b)][TEXCOORD(4b)][COLORS(4b)][NORMALS(4b)] Stride
			// 16 bits
			...
			renderer.bindAttribute("texCoordIn",
					verticesObject.asAttributeSource(VertexFormat.USHORT, 2, 16, offset + 4));
			// renderer.bindAttribute("colorIn",
			// verticesObject.asAttributeSource(VertexFormat.NORMALIZED_UBYTE, 4, 16, offset
			// + 8));
			renderer.bindAttribute("colorAndMaterialIn",
					verticesObject.asIntegerAttributeSource(VertexFormat.UBYTE, 4, 16, offset + 8, 0));
			renderer.bindAttribute("normalIn",
					verticesObject.asAttributeSource(VertexFormat.U1010102, 4, 16, offset + 12));
			break;
		case INTRICATE:
			// Complex blocks ( fp faces coordinates ) alignment :
			// Vertex data : [VERTEX_POS(12b)][TEXCOORD(4b)][COLORS(4b)][NORMALS(4b)] Stride
			// 24 bits
			renderer.bindAttribute("vertexIn", verticesObject.asAttributeSource(VertexFormat.FLOAT, 3, 24, offset + 0));
			renderer.bindAttribute("texCoordIn",
					verticesObject.asAttributeSource(VertexFormat.USHORT, 2, 24, offset + 12));
			// renderer.bindAttribute("colorIn",
			// verticesObject.asAttributeSource(VertexFormat.NORMALIZED_UBYTE, 4, 24, offset
			// + 16));
			renderer.bindAttribute("colorAndMaterialIn",
					verticesObject.asIntegerAttributeSource(VertexFormat.UBYTE, 4, 24, offset + 16, 0));
			renderer.bindAttribute("normalIn",
					verticesObject.asAttributeSource(VertexFormat.U1010102, 4, 24, offset + 20));
			break;
		default:
