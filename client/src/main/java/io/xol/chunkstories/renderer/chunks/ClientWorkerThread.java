...
/**
 * Client worker threads, with the added facility that they can do the chunk
 * rendering ( they have more buffers )
 */
public class ClientWorkerThread extends WorkerThread implements BakeChunkTaskExecutor {
	...
	class ChunkMeshingBuffers {
		...
		ChunkMeshingBuffers() {
			...
			byteBuffers = new ByteBuffer[ChunkMeshDataSubtypes.VertexLayout
					.values().length][ChunkMeshDataSubtypes.LodLevel
							.values().length][ChunkMeshDataSubtypes.ShadingType.values().length];
			;
			byteBuffersWrappers = new BaseLayoutBaker[ChunkMeshDataSubtypes.VertexLayout
					.values().length][ChunkMeshDataSubtypes.LodLevel
							.values().length][ChunkMeshDataSubtypes.ShadingType.values().length];
			;

			// Allocate dedicated sizes for relevant buffers
			byteBuffers[VertexLayout.WHOLE_BLOCKS.ordinal()][LodLevel.ANY.ordinal()][ShadingType.OPAQUE
					.ordinal()] = MemoryUtil.memAlloc(0x800000);// BufferUtils.createByteBuffer(0x800000);
			byteBuffers[VertexLayout.WHOLE_BLOCKS.ordinal()][LodLevel.LOW.ordinal()][ShadingType.OPAQUE
					.ordinal()] = MemoryUtil.memAlloc(0x400000);// BufferUtils.createByteBuffer(0x400000);
			byteBuffers[VertexLayout.WHOLE_BLOCKS.ordinal()][LodLevel.HIGH.ordinal()][ShadingType.OPAQUE
					.ordinal()] = MemoryUtil.memAlloc(0x800000);// BufferUtils.createByteBuffer(0x800000);

			byteBuffers[VertexLayout.INTRICATE.ordinal()][LodLevel.ANY.ordinal()][ShadingType.OPAQUE
					.ordinal()] = MemoryUtil.memAlloc(0x800000);// BufferUtils.createByteBuffer(0x800000);
			byteBuffers[VertexLayout.INTRICATE.ordinal()][LodLevel.LOW.ordinal()][ShadingType.OPAQUE
					.ordinal()] = MemoryUtil.memAlloc(0x400000);// BufferUtils.createByteBuffer(0x400000);
			byteBuffers[VertexLayout.INTRICATE.ordinal()][LodLevel.HIGH.ordinal()][ShadingType.OPAQUE
					.ordinal()] = MemoryUtil.memAlloc(0x800000);// BufferUtils.createByteBuffer(0x400000);
			...
			for (int i = 0; i < ChunkMeshDataSubtypes.VertexLayout.values().length; i++) {
				for (int j = 0; j < ChunkMeshDataSubtypes.LodLevel.values().length; j++) {
					for (int k = 0; k < ChunkMeshDataSubtypes.ShadingType.values().length; k++) {
						...
						default:
							pool.logger().error(
									"NO SPECIFIC LAYOUT BAKER FOR : " + ChunkMeshDataSubtypes.VertexLayout.values()[i]);
							System.exit(-400);
						}
		...
		protected final int getSunlight(Chunk c, int x, int y, int z) {
			...
			if (x >= -32 && z >= -32 && y >= -32 && y < 64 && x < 64 && z < 64) {
				...
				if (target != null) {
					...
					return c.getWorld().getContentTranslator().getVoxelForId(blockID).getDefinition().isOpaque()
							? IGNORE
							: VoxelFormat.sunlight(data);
				}
			} else {
				System.out.println("Warning ! Chunk " + c
						+ " rendering process asked information about a block more than 32 blocks away from the chunk itself");
				System.out.println(
						"This should not happen when rendering normal blocks and may be caused by a weird or buggy mod.");
				return 0;
			}
			...
			/*
			 * Chunk cached = c.getWorld().getChunk(x / 32, y / 32, z / 32); if (cached !=
			 * null && !cached.isAirChunk()) { data = cached.peekRaw(x, y, z); int blockID =
			 * VoxelFormat.id(data); return
			 * c.getWorld().getContentTranslator().getVoxelForId(blockID).getDefinition().
			 * isOpaque() ? -1 : VoxelFormat.sunlight(data); }
			 */
		...
		protected final int getBlocklight(Chunk c, int x, int y, int z) {
			...
			int blockID = VoxelFormat.id(data);
			return c.getWorld().getContentTranslator().getVoxelForId(blockID).getDefinition().isOpaque() ? IGNORE
					: VoxelFormat.blocklight(data);
		}
