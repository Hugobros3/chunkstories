...
public class DecalsVoxelBaker extends IntricateLayoutBaker implements VoxelBakerCubic {
	...
	@Override
	// Offset each vertex with the chunk location (since we draw all decals in a
	// single call and we don't keep that offset anywhere else )
	public void beginVertex(float f0, float f1, float f2) {
		super.beginVertex(cx * 32 + f0, cy * 32 + f1, cz * 32 + f2);
		/*
		 * byteBuffer.putFloat(f0 + cx * 32); byteBuffer.putFloat(f1 + cy * 32);
	...
	/*
	 * @Override public void addNormalsInt(int i0, int i1, int i2, byte extra) {
	 ...
	 * byteBuffer.putFloat((float)(i2 + 1) / 512 - 1); //this.addVerticeFloat((i0 +
	 * 1) / 512 - 1, (i1 + 1) / 512 - 1, (i2 + 1) / 512 - 1); }
	 */
}
