...
public class ArrayTextureGL extends TextureGL implements ArrayTexture {
	...
	public ArrayTextureGL(TextureFormat type, int size, int layers) {
		...
		// glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, type.getInternalFormat(), size, size,
		// layers, 0, type.getFormat(), type.getType(), (ByteBuffer)null);
		int a = size;
		int b = 0;
		while (true) {

			glTexImage3D(GL_TEXTURE_2D_ARRAY, b, type.getInternalFormat(), a, a, layers, 0, type.getFormat(),
					type.getType(), (ByteBuffer) null);
	...
	public void setMipMapping(boolean on) {
		...
		applyFiltering();
		/*
		 * if (mipmapping && !mipmapsUpToDate) computeMipmaps();
		 */
	}
	...
	protected void applyTextureParameters() {
		...
		if (mipmapping) {
			/*
			 * if (RenderingConfig.gl_openGL3Capable) GL30.glGenerateMipmap(GL_TEXTURE_2D);
	...
	/**
	 * MUST BE CALLED FROM MAIN THREAD, NO FAILSAFES WILL ALSO FUCK UP BIND POINTS,
	 * DO YOU MATH KIDS
	 */
	public void uploadTextureData(int layer, int level, ByteBuffer data) {
		...
		for (int i = 0; i < level; i++)
			sizeLod /= 2;
		glTexSubImage3D(GL_TEXTURE_2D_ARRAY, level, 0, 0, layer, sizeLod, sizeLod, 1, type.getFormat(), type.getType(),
				data);
	}
