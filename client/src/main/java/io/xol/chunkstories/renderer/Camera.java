...
public class Camera implements CameraInterface {
	...
	public void alUpdate() {
		...
		Vector3f lookAt = new Vector3f((float) (Math.sin(a) * Math.cos(b)), (float) (Math.sin(b)),
				(float) (Math.cos(a) * Math.cos(b)));

		/*
		 * Vector3f up = new Vector3f(0.0f, 1.0f, 0.0f);
		 * 
		 * lookAt.cross(up, up); up.cross(lookAt, up);
		 */

		Client.getInstance().getSoundManager().setListenerPosition((float) (double) position.x(),
				(float) (double) position.y(), (float) (double) position.z(), lookAt, up);
	}
	...
	public void updateMatricesForShaderUniforms() {
		...
		// Premultiplied versions ( optimization for poor drivers that don't figure it
		// out themselves )
	...
	private void computeFrustrumPlanes() {
		...
		Vector3f nearCenterPoint = new Vector3f((float) this.position.x, (float) this.position.y,
		...
		Vector3f farCenterPoint = new Vector3f((float) this.position.x, (float) this.position.y,
				(float) this.position.z);
		temp = new Vector3f(Z);
		temp.mul(3000f);
	...
	public boolean isBoxInFrustrum(Vector3fc center, Vector3fc dimensions) {
		...
		/*
		 * for(int i = 0; i < 2; i++) { for(int j = 0; j < 2; j++) { for(int k = 0; k <
		 * 2; k++) { corners[i * 4 + j * 2 + k].x = center.x + dimensions.x / 2f * (i ==
		 * 0 ? -1 : 1); corners[i * 4 + j * 2 + k].y = center.y + dimensions.y / 2f * (j
		 * == 0 ? -1 : 1); corners[i * 4 + j * 2 + k].z = center.z + dimensions.z / 2f *
		 * (k == 0 ? -1 : 1); } } }
		 */
	...
	private void translateCamera() {
		...
		modelViewMatrix4f.translate(
				new Vector3f((float) this.position.x, (float) this.position.y, (float) this.position.z).negate());
		computeFrustrumPlanes();
		updateMatricesForShaderUniforms();
	}
	...
	public Vector3f getViewDirection() {
		...
		float a = (float) ((180 - rotH) / 180f * Math.PI);
		float b = (float) ((-rotV) / 180f * Math.PI);
		return new Vector3f((float) (Math.sin(a) * Math.cos(b)), (float) (Math.sin(b)),
				(float) (Math.cos(a) * Math.cos(b)));
	}
	...
	public boolean isBoxInFrustrum(CollisionBox box) {
		...
		frustrumCheckBoxOrigin.set((float) (box.xpos + box.xw / 2), (float) (box.ypos + box.h / 2),
				(float) (box.zpos + box.zw / 2));
