...
public class TrueTypeFontRenderer implements FontRenderer {
	...
	/*
	 * public static TrueTypeFontRenderer get() { return trueTypeFontRenderer; }
	 */
	...
	public void drawStringIngame(TrueTypeFont trueTypeFont, float x, float y, String whatchars, float scale, int clipX,
			TextMeshObject target) {
	...
	public void drawString(Font trueTypeFont, float x, float y, String whatchars, float scaleX, float scaleY,
			float clipX, Vector4fc color) {
		drawString(trueTypeFont, x, y, whatchars, scaleX, scaleY, ALIGN_LEFT, clipX, color);
	}

	public void drawStringWithShadow(Font trueTypeFont, float x, float y, String whatchars, float scaleX, float scaleY,
			Vector4fc color) {
	...
	public void drawStringWithShadow(Font trueTypeFont, float x, float y, String whatchars, float scaleX, float scaleY,
		...
		drawString(trueTypeFont, x + 1 * scaleX, y - 1 * scaleY, whatchars, scaleX, scaleY, ALIGN_LEFT, clipX,
				colorDarkened);
		drawString(trueTypeFont, x, y, whatchars, scaleX, scaleY, ALIGN_LEFT, clipX, color);
	}
	...
	public void drawString(Font trueTypeFont, float x, float y, String whatchars, float scaleX, float scaleY,
			int alignement, float clipX, Vector4fc color) {
		drawString(trueTypeFont, x, y, whatchars, scaleX, scaleY, alignement, clipX, color, null);
	}

	private void drawString(Font font, float x, float y, String whatchars, float scaleX, float scaleY, int alignement,
			float clipX, Vector4fc color, TextMeshObject target) {
		...
		if (alignement == ALIGN_CENTER) {
			// System.out.println(clipX + " " + currentLineTotalLength + " -> " + (clipX /
			// scaleX - currentLineTotalLength) / 2);
			...
			if (glyph != null) {
				...
				if (charCurrent == '#' && whatchars.length() - i - 1 >= 6 && (whatchars.toCharArray()[i + 1] != '#')
					...
					if (!(i > 1 && whatchars.toCharArray()[i - 1] == '#')) {
						...
						colorModified = new Vector4f(rgb[0] / 255.0f * color.x(), rgb[1] / 255.0f * color.y(),
					...
					if (target == null) {
						// renderingContext.getGuiRenderer().setState(pageTexture, true, true,
						// colorModified);
						drawQuad(totalwidth * scaleX + x, startY * scaleY + y, (glyph.width) * scaleX,
								glyph.height * scaleY, ((float) glyph.x) / textureWidth,
								((float) (glyph.y + glyph.height)) / textureHeight,
								((float) (glyph.x + glyph.width)) / textureWidth, ((float) glyph.y) / textureHeight,
								pageTexture, colorModified);
					} else {
						target.setState(pageTexture, colorModified);
						target.drawQuad(totalwidth * scaleX + x, startY * scaleY + y, (glyph.width) * scaleX,
								glyph.height * scaleY, ((float) glyph.x) / textureWidth,
								((float) (glyph.y + glyph.height)) / textureHeight,
								((float) (glyph.x + glyph.width)) / textureWidth, ((float) glyph.y) / textureHeight);
					}
	...
	private void drawQuad(float startX, float startY, float width, float height, float srcX, float srcY, float srcX2,
			float srcY2, Texture2DGL pageTexture, Vector4f colorModified) {
		renderingContext.getGuiRenderer().drawBoxWindowsSpace(startX, startY, startX + width, startY + height, srcX,
				srcY, srcX2, srcY2, pageTexture, false, true, colorModified);
	}
		...
		if (font == null) {
			try {
				font = new TrueTypeFont(
						this.renderingContext.getClient().getContent().getAsset("./font/" + fontName + ".ttf"),
						sizeInPX, aa);
