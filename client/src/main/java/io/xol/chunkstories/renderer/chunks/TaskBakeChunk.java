...
public class TaskBakeChunk extends Task {
	...
	protected boolean task(TaskExecutor taskExecutor) {
		...
		Vector3dc camera = ((WorldClient) chunk.getWorld()).getWorldRenderer().getRenderingInterface().getCamera()
				.getCameraPosition();

		// Check we aren't too far from the camera, and thus that our request hasn't
		// been yet cancelled
		...
		if (dx > chunksViewDistance || dz > chunksViewDistance || dy > 2) {
			// logger.info("unscheduled chunk mesh render task for it being too far to be
			// rendered anyway");
			return true;
		}

		// Require the chunk and nearby ones to be already loaded in the world
		ChunkRenderable chunkWithinWorld = (ChunkRenderable) world.getChunk(chunk.getChunkX(), chunk.getChunkY(),
				chunk.getChunkZ());
		if (chunkWithinWorld != null) {
			...
			if (world.isChunkLoaded(chunk.getChunkX(), chunk.getChunkY(), chunk.getChunkZ() - 1))
				nearChunks++;
			if (world.isChunkLoaded(chunk.getChunkX(), chunk.getChunkY() + 1, chunk.getChunkZ())
					|| chunk.getChunkY() == world.getWorldInfo().getSize().heightInChunks - 1)
				nearChunks++;
			if (world.isChunkLoaded(chunk.getChunkX(), chunk.getChunkY() - 1, chunk.getChunkZ())
					|| chunk.getChunkY() == 0)
				nearChunks++;
			...
			// We wait until the chunk is loaded in the world ( or destroyed, then the task
			// is cancelled )
			return false;
		}
		...
		// Fill chunk caches ( saves much time avoiding slow-ass world->regions
		// hashmap->chunk holder access for each vert )
		...
		ChunkRenderer chunkRendererOutput = new ChunkRenderer() {
			...
			public VoxelBakerHighPoly getHighpolyBakerFor(LodLevel lodLevel, ShadingType renderPass) {
				return (VoxelBakerHighPoly) cmd.byteBuffersWrappers[VertexLayout.INTRICATE.ordinal()][lodLevel
						.ordinal()][renderPass.ordinal()];
			}

			@Override
			public VoxelBakerCubic getLowpolyBakerFor(LodLevel lodLevel, ShadingType renderPass) {
				return (VoxelBakerCubic) cmd.byteBuffersWrappers[VertexLayout.WHOLE_BLOCKS.ordinal()][lodLevel
						.ordinal()][renderPass.ordinal()];
			}
		...
		for (i = 0; i < 32; i++) {
			for (j = 0; j < 32; j++) {
				for (k = 0; k < 32; k++) {
					...
					// We handle voxels with a dynamic renderer here too - we just add them to a
					// list !
		...
		// Parse output neatly
		int[][][] sizes = new int[ChunkMeshDataSubtypes.VertexLayout.values().length][ChunkMeshDataSubtypes.LodLevel
				.values().length][ChunkMeshDataSubtypes.ShadingType.values().length];
		;
		int[][][] offsets = new int[ChunkMeshDataSubtypes.VertexLayout.values().length][ChunkMeshDataSubtypes.LodLevel
				.values().length][ChunkMeshDataSubtypes.ShadingType.values().length];
		;
		...
		for (VertexLayout vertexLayout : VertexLayout.values())
			for (LodLevel lodLevel : LodLevel.values())
				for (ShadingType renderPass : ShadingType.values()) {
					...
					sizes[vertexLayoutIndex][lodLevelIndex][renderPassIndex] = relevantByteBuffer.position()
		...
		for (int j = 0; j < dynamicVoxels.size(); j++) {
			if (i.hasNext())
				...
				logger.error(
						"while baking dynamicVoxelTypes array the iterator returned less than dynamicVoxels.size() elements");
		...
		// Wait until data is actually uploaded to not accidentally OOM while it
		// struggles uploading it
		// if(Client.getInstance().configDeprecated().getBoolean("waitForChunkMeshDataUploadBeforeStartingTheNext",
		// true))
		...
		public ChunkBakerRenderContext(CubicChunk chunk, int cx, int cy, int cz) {
			...
			voxelLighter = new VoxelLighter() {
				...
				private byte interp(byte[] array, float x, float y, float z) {
					float interpBotFront = Math2.mix(array[VoxelSide.Corners.BOTTOM_FRONT_LEFT.ordinal()],
							array[VoxelSide.Corners.BOTTOM_FRONT_RIGHT.ordinal()], Math2.clamp(x, 0.0, 1.0));
					float interpBotBack = Math2.mix(array[VoxelSide.Corners.BOTTOM_BACK_LEFT.ordinal()],
							array[VoxelSide.Corners.BOTTOM_BACK_RIGHT.ordinal()], Math2.clamp(x, 0.0, 1.0));

					float interpTopFront = Math2.mix(array[VoxelSide.Corners.TOP_FRONT_LEFT.ordinal()],
							array[VoxelSide.Corners.TOP_FRONT_RIGHT.ordinal()], Math2.clamp(x, 0.0, 1.0));
					float interpTopBack = Math2.mix(array[VoxelSide.Corners.TOP_BACK_LEFT.ordinal()],
							array[VoxelSide.Corners.TOP_BACK_RIGHT.ordinal()], Math2.clamp(x, 0.0, 1.0));
