...
public class CubemapGL extends TextureGL implements Cubemap {
	...
	public void bind() {
		...
		// Don't bother
		if (glId == -2) {
			logger().error("Critical mess-up: Tried to bind a destroyed Cubemap " + this
	...
	public int loadCubemapFromDisk() {
		...
		try {
			for (int i = 0; i < 6; i++) {
				...
				glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, type.getInternalFormat(), decoder.getWidth(),
						decoder.getHeight(), 0, type.getFormat(), type.getType(), temp);
				// Anti alias
				glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
				glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
				// Anti seam
	...
	public class Face implements RenderTarget {
		...
		public Face(int i) {
			...
			glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, type.getInternalFormat(), size, size, 0,
					type.getFormat(), type.getType(), (ByteBuffer) null);
		}
