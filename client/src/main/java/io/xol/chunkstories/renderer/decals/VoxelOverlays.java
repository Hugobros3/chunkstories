...
public class VoxelOverlays {
	...
	public void drawSelectionBox(RenderingInterface renderingInterface, Location location) {
		...
		for (CollisionBox box : boxes) {
			i = cubeVertices(data, i, (float) box.xpos, (float) box.ypos, (float) box.zpos, (float) box.xw,
					(float) box.h, (float) box.zw);
		}
		...
		/*
		 * glColor4f(1, 1, 1, 1.0f); //GL11.glBlendFunc(GL11.GL_ONE_MINUS_SRC_COLOR,
		 * GL11.GL_ZERO); //GL11.glBlendFunc(GL11.GL_ONE, GL11.GL_ONE);
		 * //GL11.glBlendEquation(GL11.); glBegin(GL_LINES); VoxelContext bri = new
		 * VoxelContextOlder(world, x, y, z); if (bri.getVoxel() == null) {
		 * System.out.println(bri.getData()); return; } for (CollisionBox box :
		 * bri.getVoxel().getTranslatedCollisionBoxes(world, x, y, z))
		 * cubeVertices((float) box.xpos, (float) box.ypos, (float) box.zpos, (float)
		 * box.xw, (float) box.h, (float) box.zw); glEnd(); glColor4f(1, 1, 1, 1);
		 */
	}
	...
	private int addPoints(ByteBuffer data, int i, float x, float y, float z) {
		/*
		 * data[i] = x; i++; data[i] = y; i++; data[i] = z; i++;
		 */
		data.putFloat(x);
		data.putFloat(y);
		data.putFloat(z);
