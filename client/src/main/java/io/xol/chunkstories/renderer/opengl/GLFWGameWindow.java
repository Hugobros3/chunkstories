...
public class GLFWGameWindow implements GameWindow {
	...
	private void createOpenGLContext() {
		logger().info("Creating an OpenGL Windows [title:" + windowName + ", width:" + windowWidth + ", height:"
				+ windowHeight + "]");
		try {
			computeDisplayModes();
			...
			// Oops.. Didn't use any VAOs anywhere so we put this there to be GL 3.2 core
			// compliant
	...
	private void displaySplashScreen() throws IOException {
		...
		// glUniform2f(uniformLocation, ((Vector2fc)uniformData).x(),
		// ((Vector2fc)uniformData).y());
	...
	private void systemInfo() {
		...
		if (allocatedRam < 1024 * 1024 * 1024L) {
			...
			JOptionPane.showMessageDialog(null,
					"Not enought ram, we will offer NO support for crashes and issues when launching the game with less than 1Gb of ram allocated to it."
	...
	private void glInfo() {
		...
		logger().info("Render device :" + glGetString(GL_RENDERER) + " vendor:" + glGetString(GL_VENDOR) + " version:"
				+ glVersion);
	...
	public void run() {
		try {
			...
			while (glfwWindowShouldClose(glfwWindowHandle) == false && !closeRequest) {
				...
				/*
				 * for (Runnable r : mainThreadQueue) r.run(); mainThreadQueue.clear();
				 */
	...
	private void computeDisplayModes() {
		...
		while (pb.remaining() > 0) {
			...
			String monitorName = "" + monitorCount + ": " + (mainMonitor == monitorHandle ? " (Main)" : "") + " "
					+ glfwGetMonitorName(monitorHandle);
			...
			while (videoModes.remaining() > 0) {
				...
				// String videoModeString = videoMode.width() + "x" + videoMode.height() + " @"
				// + videoMode.refreshRate()+"Hz ";

				// System.out.println(videoModeString+(videoMode.blueBits()+videoMode.redBits()+videoMode.greenBits())
				// + "bpp");
	...
	private void switchResolution() {
		...
		boolean isFullscreenEnabled = client.getConfiguration() == null ? false
				: client.getConfiguration().getBooleanOption("client.video.fullScreen");
		if (isFullscreenEnabled) {
			// Enable fullscreen using desktop resolution, by default on the primary monitor
			// and at it's nominal video mode
			...
			if (modeString == null || modeString.contains("x") || !modeString.contains(":")) {
				modeString = "1:" + currentVideoMode.width() + ":" + currentVideoMode.height() + ":"
			...
			glfwSetWindowMonitor(this.glfwWindowHandle, monitors[videoMode.monitorId - 1], 0, 0,
					videoMode.videoMode.width(), videoMode.videoMode.height(), videoMode.videoMode.refreshRate());
			...
			glfwSetWindowMonitor(this.glfwWindowHandle, MemoryUtil.NULL, (currentVideoMode.width() - defaultWidth) / 2,
					(currentVideoMode.height() - defaultHeight) / 2, defaultWidth, defaultHeight, GLFW_DONT_CARE);
	...
	public void toggleFullscreen() {
		...
		client.getConfiguration().getOption("client.video.fullScreen")
				.trySetting(isFullscreenEnabled ? "true" : "false");
		// client.configDeprecated().setString("fullscreen", isFullscreenEnabled ?
		// "true" : "false");
		switchResolution();
	}
	...
	private DisplayMode findMatchForVideoMode(String modeString) {
		...
		for (DisplayMode v : enumeratedVideoModes) {
			if (v.monitorId == id && v.videoMode.width() == w && v.videoMode.height() == h
					&& v.videoMode.refreshRate() == freq)
				return v;
		}
	...
	public Fence queueSynchronousTask(Runnable runnable) {
		/*
		 * synchronized (mainThreadQueue) { mainThreadQueue.add(runnable); }
		 */
	...
	public String takeScreenshot() {
		...
		ByteBuffer bbuf = MemoryUtil.memAlloc(4 * scrW * scrH);// ByteBuffer.allocateDirect(scrW * scrH *
