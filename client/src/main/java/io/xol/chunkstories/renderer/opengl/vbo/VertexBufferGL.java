...
public class VertexBufferGL implements VertexBuffer {
	...
	public enum UploadRegime {
		FAST(GL_STREAM_DRAW), ONCE(GL_STATIC_DRAW);
	...
	public void bind() {
		if (glID == -2) {
			logger().error(
					"Critical mess-up: Tried to bind a destroyed VerticesObject. Terminating process immediately.");
			// logger().save();
			Thread.dumpStack();
			System.exit(-800);
	...
	private Fence setAsyncDataPendingUpload(Object o) {
		...
		if (alreadyPendingUpload != null) {
			...
			// Recycle any pooled byte buffer that won't be uploaded ( nice guys always
			// loose )
	 ...
	 * 
	 * @see
	 * io.xol.engine.graphics.geometry.VertexBuffer#uploadData(java.nio.ByteBuffer)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * io.xol.engine.graphics.geometry.VertexBuffer#uploadData(java.nio.FloatBuffer)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * io.xol.engine.graphics.geometry.VertexBuffer#uploadData(io.xol.chunkstories.
	 * renderer.buffers.ByteBufferPool.RecyclableByteBuffer)
	 */
	@Override
	public Fence uploadData(RecyclableByteBuffer dataToUpload) {
		...
		if (Client.getInstance().getGameWindow().isMainGLWindow()) {
			...
			if (replacing != null && replacing != dataToUpload && replacing instanceof RecyclableByteBuffer) {
				// System.out.println("Watch out, uploading two RecyclableByteBuffer in a row,
				// the first one is getting recycled early to prevent locks");
	...
	private boolean uploadDataActual(Object dataToUpload) {
		...
		if (glID == -2) {
			System.out
					.println("FATAL: Attempted to upload data to a destroy()ed VertexBuffer. Terminating immediately.");
	...
	private void uploadPendingDefferedData() {
		...
		if (quickCheck != null) {
			...
			// this should NEVER be null, the only code path able to null this out is this
			// very block, and it's only called from a single thread ever
			// (the graphics thread). If this fails, we might as well crash and burn the
			// entire world
	...
	class VerticesObjectAsAttribute implements AttributeSource {
		...
		public VerticesObjectAsAttribute(VertexFormat format, int dimensions, int stride, long offset, int divisor,
				boolean asIntAttribute) {
			this.format = format;
		...
		public void setup(int gl_AttributeLocation) {
			...
			if (!isDataPresent())
				throw new RuntimeException(
						"No VBO data uploaded | " + Client.getInstance().getGameWindow().getRenderingInterface());
			// Set pointer
			if (asIntAttribute)
