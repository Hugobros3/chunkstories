...
/**
 * Processes shaders 'include' statements and appends them a bunch of defines
 * matching the client.rendering options
 */
public class GLSLPreprocessor {
	public static void loadRecursivly(ModsManager modsManager, Asset asset, StringBuilder into, boolean type,
			Set<String> alreadyIncluded) throws IOException {
				...
				if (line.length == 2) {
					if (line[0].equals("include")) {
						...
						while (shaderInclude.indexOf("..") != -1) {
							...
							if (indexOf <= 0) {
								System.out.println(
										"Irresolvable path; going too far back in parents folder; " + shaderInclude);
								break;
							}
						...
						if (alreadyIncluded.add(asset2include.getName()))
							...
							System.out.println(
									"Shader include : " + asset2include.getName() + " already included, skipping");
		...
		for (Option option : Client.getInstance().getConfiguration().allOptions()) {
			...
			if (fullname.startsWith("client.rendering")) {
				...
				if (option instanceof OptionBoolean) {
					...
					// shaderSource.append("#define " + abridgedName + " " +
					// (option.getValue().equals("true") ? 1 : 0) + "\n");
				} else
					shaderSource.append("#define " + abridgedName + " " + option.getValue().toString() + "\n");
				// System.out.println("#define " + abridgedName + " " +
