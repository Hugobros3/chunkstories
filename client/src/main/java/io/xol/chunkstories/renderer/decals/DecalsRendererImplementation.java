	...
	class DecalType {
		...
		void addDecal(Vector3dc position, Vector3dc orientation, Vector3dc size) {
				...
				for (int x = 0; x < size2.x(); x++)
					for (int y = 0; y < size2.y(); y++)
						for (int z = 0; z < size2.z(); z++) {
							...
							if (voxel != null && !cell.getVoxel().isAir() && !voxel.getDefinition().isLiquid()
								...
								ChunkRenderer chunkRenderer = new ChunkRenderer() {
									...
									@Override
									public VoxelBakerHighPoly getHighpolyBakerFor(LodLevel lodLevel,
											ShadingType renderPass) {
										return virtualRenderBytebuffer;
									}

									@Override
									public VoxelBakerCubic getLowpolyBakerFor(LodLevel lodLevel,
											ShadingType renderPass) {
										return virtualRenderBytebuffer;
									}
								...
								ChunkRenderer.ChunkRenderContext o2 = new ChunkRenderer.ChunkRenderContext() {
									...
									private VoxelLighter voxeLighter = new VoxelLighter() {
										...
										@Override
										public byte getSunlightLevelInterpolated(float vertX, float vertY,
												float vertZ) {
											return 0;
										}

										@Override
										public byte getBlocklightLevelInterpolated(float vertX, float vertY,
												float vertZ) {
											return 0;
										}
			...
			if (c > 0.0) {
				...
				actualCount += TrianglesClipper.clipTriangles(bbuf, decalsByteBuffer, rotationMatrix, position, lookAt,
						size);
	...
	public void renderDecals(RenderingInterface renderingInterface) {
		for (DecalType decalType : decalsTypes.values()) {
			...
			renderingInterface.bindAttribute("vertexIn",
					decalType.verticesObject.asAttributeSource(VertexFormat.FLOAT, 3, 4 * (3 + 2), 0));
			renderingInterface.bindAttribute("texCoordIn",
					decalType.verticesObject.asAttributeSource(VertexFormat.FLOAT, 2, 4 * (3 + 2), 4 * 3));
