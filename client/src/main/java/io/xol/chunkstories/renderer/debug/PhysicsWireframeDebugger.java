...
public class PhysicsWireframeDebugger {
	...
	public void render(RenderingInterface renderer) {
		...
		for (int i = ((int) (double) cameraPosition.x()) - drawDebugDist; i <= ((int) (double) cameraPosition.x())
				+ drawDebugDist; i++)
			for (int j = ((int) (double) cameraPosition.y()) - drawDebugDist; j <= ((int) (double) cameraPosition.y())
					+ drawDebugDist; j++)
				for (int k = ((int) (double) cameraPosition.z())
						- drawDebugDist; k <= ((int) (double) cameraPosition.z()) + drawDebugDist; k++) {
		...
		while (ie.hasNext()) {
			...
			if (e.getTranslatedBoundingBox().lineIntersection(cameraPosition,
					new Vector3d(renderer.getCamera().getViewDirection())) != null)
				FakeImmediateModeDebugRenderer.renderCollisionBox(e.getTranslatedBoundingBox(),
						new Vector4f(0, 0, 0.5f, 1.0f));
			else
				FakeImmediateModeDebugRenderer.renderCollisionBox(e.getTranslatedBoundingBox(),
						new Vector4f(0, 1f, 1f, 1.0f));
