...
/**
 * Idea: stop with the idea of building meshes on the CPU. Just use pre-computed
 * grids or a geometry shader and a big array texture with all the summaries to
 * draw this shit insanely fast.
 */
public class FarTerrainGSMeshRenderer implements FarTerrainRenderer {
	...
	VertexBufferGL generateGrid(int gridSubdivisions) {
		...
		for (int a = 0; a < gridSubdivisions + 1; a++)
			for (int b = 0; b < gridSubdivisions + 1; b++) {
				...
				/*
				 * bb.putFloat(i + s); bb.putFloat(0); bb.putFloat(j + s);
				 * 
				 * bb.putFloat(i + s); bb.putFloat(0); bb.putFloat(j + 0);
				 * 
				 * bb.putFloat(i + 0); bb.putFloat(0); bb.putFloat(j + 0);
				 * 
				 * bb.putFloat(i + 0); bb.putFloat(0); bb.putFloat(j + s);
				 * 
				 * bb.putFloat(i + s); bb.putFloat(0); bb.putFloat(j + s);
				 */
			}
	...
	public void renderTerrain(RenderingInterface renderer, ReadyVoxelMeshesMask mask) {
		...
		if (!renderer.getClient().getInputsManager().getInputByName("hideFarTerrain").isPressed()
			...
			// MemFreeByteBuffer auto_free_summariesAttributes = new
			// MemFreeByteBuffer(summariesAttributes);
			// int count = 0;
			...
			for (int i = -4; i <= 4; i++)
				for (int j = -4; j <= 4; j++) {
					...
					HeightmapImplementation sum = (HeightmapImplementation) world.getRegionsSummariesHolder()
							.getHeightmap(regionI, regionJ);

					// Early out
					if (sum == null || !sum.isLoaded())
						continue;

					if (!renderer.getCamera().isBoxInFrustrum(new CollisionBox(regionI * 256, 0, regionJ * 256, 256,
							1024 /* + sum.getHeightMipmapped(0, 0, 9) */, 256)))
						continue;
					...
					for (int l = 0; l < 8; l++)
						for (int m = 0; m < 8; m++) {
							...
							int lod = detailLevels.length - (int) ((lodBias
									+ Math.pow(Math.min(1024, center.distance(playerCenter)) / 1024, lodExponent))
									* detailLevels.length);
							// System.out.println((Math.min(512, center.distance(playerCenter)) / 512f *
							// detailLevels.length));
							 ...
							 * 
							 * if(summariesAttributes == null) { summariesAttributes = MemoryUtil.memAlloc(9
							 * * 9 * 8 * 8 * (4 + 2 * 4)); lodByteBuffer[lod] = summariesAttributes; }
							 * 
							 * summariesAttributes.putFloat(regionI * 256 + l * 32);
							 * summariesAttributes.putFloat(regionJ * 256 + m * 32);
							 * summariesAttributes.put((byte)index); summariesAttributes.put((byte)index10);
							 * summariesAttributes.put((byte)index01);
							 * summariesAttributes.put((byte)index11); lodInstanceCount[lod]++; }
							 */
						}
				}
			...
			for (int lod = detailLevels.length - 1; lod >= 0; lod--) {
				...
				renderer.bindAttribute("displacementIn",
						gridAttributes.asAttributeSource(VertexFormat.FLOAT, 2, (4 + 2 * 4), 0L, 1));
				renderer.bindAttribute("indexIn",
						gridAttributes.asIntegerAttributeSource(VertexFormat.BYTE, 4, (4 + 2 * 4), 8L, 1));

				renderer.draw(Primitive.POINT, 0, (detailLevels[lod] + 1) * (detailLevels[lod] + 1) * 1,
						lodInstanceCount[lod]);
			}
		}
