...
public class ALSoundSource extends SoundSourceAbstract {
	...
	public void play() {
		...
		if (soundData == null)
			...
			if (soundData instanceof SoundDataBuffered) {
				...
				// Upload the first two pages, the first one is set to be the first one we'll
				// swap
	...
	public void stop() {
		...
		/*
		 * if(efxSlot != -1) alDeleteAuxiliaryEffectSlots(efxSlot);
		 */
	}
	...
	private void updateSource() {
		...
		if (updateProperties) {
			...
			// TODO EFX LWJGL3
			/*
			 * if (effectSlotId != -1) alSource3i(openAlSourceId, AL_AUXILIARY_SEND_FILTER,
			 * effectSlotId, 0, AL_FILTER_NULL); else alSource3i(openAlSourceId,
