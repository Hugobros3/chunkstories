...
void main() {
	...
	for(int i = 0; i < lightsToRender; i++)
	{
		...
		if(spec > 0)
		{
			lightAmount.rgb += lightColor[i]  * clamp(pow(clamp(dot(reflect(lightRay, normalWorld), normalize(pixelPositionWorld-camPos)), 0.0, 10.0), 1000), 0.0, 10.0);
		}
		if(lightAngle[i] > 0)
		{
	...
	gl_FragColor = totalLight * texture2D(albedoBuffer, screenCoord);
}
