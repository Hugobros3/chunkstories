...
// All the characters that are allowed in a getRegisteredComponentName (excl the first char that can only be a LETTER)
...
// A getRegisteredComponentName stars with a letter and then some characters
Name: LETTER (NAME_CHARACTERS)*;

// Unquoted text is like a getRegisteredComponentName but *has* to contain a special character (avoids conflicts with Name)
