...
public class PacketsStoreOld// implements Content.PacketTypes
{
	private final GameContentStore store;
	private final ModsManager modsManager;
	
	public PacketsStoreOld(GameContentStore store)
	{
		this.store = store;
	...
	private void loadPacketFile(Asset f)
	{
		...
		ChunkStoriesLoggerImplementation.getInstance().log("Loading packets in " + f);
	...
	class PacketTypeLoaded
	{
		...
		public Packet createNew() throws IllegalPacketException
		{
			...
			{
				ChunkStoriesLoggerImplementation.getInstance().warning("Failed to instanciate " + packetName);
				e.printStackTrace();
			}
			return null;
		}
