		...
		try (MemoryStack stack = MemoryStack.stackPush()) {
			...
			for(int mesh = 0; mesh < meshesN; mesh++) {
				...
				for(int j = 0; j < bonesN; j++) {
					...
					/*for(int k = 0; k < weightsN; k++) {
						AIVertexWeight vertexWeight = bone.mWeights().get(k);
						System.out.println(vertexWeight.mWeight() + ": "+vertexWeight.mVertexId());
					}*/
				}
		...
		for(int i = 10000; i < materialsNum; i++) {
			...
			for(int j = 0; j < propertiesNum; j++) {
				...
				case Assimp.AI_BOOL:
					...
					break;
				case Assimp.AI_INT:
					int intValue = property.mData().getInt(0);
					System.out.println("Int: "+intValue);
					break;
				case Assimp.AI_UINT64:
					long longValue = property.mData().getLong(0);
					System.out.println("Long: "+longValue);
					break;
				case Assimp.AI_FLOAT:
					float floatValue = property.mData().getFloat(0);
					System.out.println("Float: "+floatValue);
					break;
				case Assimp.AI_AIVECTOR3D:
					Vector3f vec3Value = new Vector3f();
					vec3Value.x = property.mData().getFloat(0);
					vec3Value.y = property.mData().getFloat(4);
					vec3Value.z = property.mData().getFloat(8);
					System.out.println("Vec3: "+vec3Value);
					break;
